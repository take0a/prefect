...
`prefect-dbt` を使用すると、dbt Cloud ジョブをトリガーおよび監視したり、dbt Core CLI コマンドを実行したり、[Snowflake](/integrations/prefect-snowflake/index) などの他のツールを dbt 実行に組み込んだりできます。
Prefect はワークフローの状態をグローバルに表示し、状態の変化に基づいてアクションを実行できます。

Prefect 統合では、外部システムとやり取りするための、事前に構築された [ブロック](/v3/develop/blocks)、[フロー](/v3/develop/write-flows)、または [タスク](/v3/develop/write-tasks) が提供される場合があります。
このライブラリのブロックタイプを使用すると、dbt Cloud ジョブの実行や dbt Core コマンドの実行などを行うことができます。
...
<Info>
Resources in Prefect Cloud are currently experimental. Enable them by setting the `PREFECT_EXPERIMENTS_LINEAGE_EVENTS_ENABLED` environment variable to `true` wherever flow runs are executed.
</Info>

The `PrefectDbtRunner` class automatically emits lineage events describing the relationships between the resources in your dbt project.
...
To describe resource that exist outside of your dbt project and are upstream of a given node, supply the resource in your dbt config.
For example, to describe a BigQuery table as an upstream resource of a dbt model, add the following to your model's config:
